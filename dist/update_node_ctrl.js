...
System.register(['./utils'], function (_export, _context) {
  ...
  var utils, filter;
  function updateNode(node, allData, panelCtrl) {
  ...
  function prepareModalData(node, allData) {
    ...
    if (node.type === 'Category') {
      ...
      self = 'Reason';
      filter = allData.filter(function (d) {
        return d.category_id === node.info.category && d.reason_id !== null && d.parent_reason_id === null;
      });
      filter = filter.reduce(function (arr, record) {
      ...
      self = 'Sub-Reason';
      filter = allData.filter(function (d) {
        return d.category_id === node.info.category && d.reason_id !== null && d.parent_reason_id === node.parent;
      });
      filter = filter.reduce(function (arr, record) {
        arr.push(record.reason_id);
        return arr;
  ...
  function startUpdate(input, node, panelCtrl, allData) {
    ...
    if (!isInputValidSecondCheck(input, node, allData)) {
      return;
    }
  ...
  function updateForReasons(url, line, childUrl, childLine, panelCtrl) {
    utils.update(url, line).then(utils.update(childUrl, childLine).then(function (res) {
      //   console.log(res)
      $('#master-data-reason-code-update-node-cancelBtn').trigger('click');
      utils.alert('success', 'Success', 'A new node has been succeesfully inserted');
      panelCtrl.refresh();
  ...
  function isInputValidSecondCheck(input, node, allData) {
    ...
    if (node.type === 'Parent Reason' || node.type === 'Reason') {
      ...
      var reasons = utils.getReasons(node, allData);
...
//# sourceMappingURL=update_node_ctrl.js.map
