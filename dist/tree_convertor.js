...
System.register([], function (_export, _context) {
  ...
  function getTreeStructureData(data) {
    ...
    };var distinctSites = findDisctinct(data, 'category');

    for (var i = 0; i < distinctSites.length; i++) {
      var category = distinctSites[i];
      var ob = {
        name: category,
        children: [],
        // collapsed: false,
  ...
  function getReasons(obj, data) {
    var _loop = function _loop(i) {
      var category = obj.children[i];
      //find areas that are under this site
      var reasons = data.filter(function (d) {
        return d.category_id !== null && d.category_id === category.name && d.parent_reason_id === null && d.reason_id !== null;
      });
      //all areas to distinct areas
      var distinctReasons = findDisctinct(reasons, 'reason');
      //add each distinct area to this site
      for (var k = 0; k < distinctReasons.length; k++) {
        var reason = distinctReasons[k];
        var ob = {
          name: reason,
          children: [],
          collapsed: false,
  ...
  function addReasons(node, category, data) {
    ...
    var childReasons = data.filter(function (d) {
      return d.reason_id !== null && d.parent_reason_id !== null && d.category_id === category.name && d.parent_reason_id === node.name;
    });
    if (childReasons.length > 0) {
      //node's children to node's disctinct children
      var _distinctReasons = findDisctinct(childReasons, 'reason');
      //for each children
      for (var i = 0; i < _distinctReasons.length; i++) {
        var reason = _distinctReasons[i];
        //init this child
        var child = {
          name: reason,
          children: [],
